...
export class Router implements IRouter {
    ...
    if (route?.auth) {
      if (!request.headers["authorization"]) {
        return this.handleResponse(
          request,
          response,
          Result.fail(unauthorized("Token de requisição não encontrado!"))
        );
      }
      ...
      if (splitToken[0] !== "Bearer" || !splitToken[1]) {
        return this.handleResponse(
          request,
          response,
          Result.fail(unauthorized("Token inválido!"))
        );
      }
      ...
      if (!jwtService.verify(splitToken[1]))
        return this.handleResponse(
          request,
          response,
          Result.fail(unauthorized("Falha de autenticação!"))
        );
    }
  }
    ...
    request.on("end", async () => {
      if (method === "POST" && !request?.body) {
        this.handleResponse(
          request,
          response,
          Result.fail(badRequest("Corpo de requisição vazio!"))
        );
      }
      try {
        ...
        if (error instanceof BaseException) {
          ...
          this.handleResponse(
            request,
            response,
            Result.fail(internalServerError(error?.message))
          );
        }
      }
    });
  ...
  private findRoute(path: string, method: string): IRoute | undefined {
    const route = this.routes?.find((e) => {
      return e.path === path && e.method.toUpperCase() === method;
    });
